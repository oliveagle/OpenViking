...
| Cross-platform binaries | `vX.Y.Z` (both) / `lib@X.Y.Z` (lib only) / `cli@X.Y.Z` (CLI only) | The `Release Binaries` workflow publishes per-platform `-cli` / `-lib` archives; `lib@*` is the Go/cgo embedding track. |
...
The binary release supports independent tracks (`.github/workflows/release-binaries.yml`):

- **Full release** (`vX.Y.Z`): builds and publishes both the `ov` CLI and the `ragfs-ffi` staticlib per platform. Release name: `OpenViking vX.Y.Z`.
- **Lib only** (`lib@X.Y.Z`): publishes only the `ragfs-ffi` staticlib + C-header archives. Release name: `OpenViking Lib X.Y.Z` — the track Go/cgo embedders want (see `docs/embedding-go.md`).
- **CLI only** (`cli@X.Y.Z`): publishes only the `ov` CLI archives, alongside the npm release. Release name: `OpenViking CLI X.Y.Z`.

Archives are per-platform (`linux/amd64`, `linux/arm64`, `macos/amd64`, `macos/arm64`, `windows/amd64`) as `openviking-<version>-<platform>-{cli,lib}.{tar.gz,zip}`, plus a `SHA256SUMS` checksum file.
