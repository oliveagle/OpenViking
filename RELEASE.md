...
| Cross-platform binaries | `v<hash>` (both) / `lib@<hash>` (lib only) / `cli@<hash>` (CLI only) / `d@<hash>` (daily) | The `Release Binaries` workflow publishes per-platform `-cli` / `-lib` archives; the version is the short git hash of the release commit (no semantic versioning); `lib@*` is the Go/cgo embedding track. |
...
## Cross-Platform Binary Release Flow

The binary release version is the **short git hash of the release commit** (no semantic versioning); tags are plain triggers (`.github/workflows/release-binaries.yml`):

- **Full release** (tag `v<hash>`): builds and publishes both the `ov` CLI and the `ragfs-ffi` staticlib per platform. Release name: `OpenViking <hash>`.
- **Lib only** (tag `lib@<hash>`): publishes only the `ragfs-ffi` staticlib + C-header archives. Release name: `OpenViking Lib <hash>` — the track Go/cgo embedders want (see `docs/embedding-go.md`).
- **CLI only** (tag `cli@<hash>`): publishes only the `ov` CLI archives, alongside the npm release. Release name: `OpenViking CLI <hash>`.

Archives are per-platform (`linux/amd64`, `linux/arm64`, `macos/amd64`, `macos/arm64`, `windows/amd64`) as `openviking-<hash>-<platform>-{cli,lib}.{tar.gz,zip}`, plus a `SHA256SUMS` checksum file.

The workflow also supports manual dispatch with a `component` input (`all` / `cli` / `lib`) for ad-hoc builds without a tag.

### Daily Sync & Release

`.github/workflows/daily-release.yml` runs daily (02:00 Asia/Shanghai): it merges the upstream `main` into the sync branch (default `feat/cross-platform-release-binaries`, configurable via the repo variable `DAILY_SYNC_BRANCH`), pushes the merge commit, then calls `release-binaries.yml` to publish a full release tagged `d@<hash>` and named `OpenViking <hash>`.

> Note: GitHub does not run `schedule` triggers on forked repos. While this repo is a fork, drive the daily run from a local cron — `0 2 * * * cd <repo> && scripts/daily-local.sh` (or manually: `gh workflow run daily-release.yml --ref <branch>`). Once the repo is standalone (or merged upstream), the schedule takes over.
